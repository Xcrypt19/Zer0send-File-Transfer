(function(){
    ...
    // Pack: prepend 24-byte ASCII fileId to a binary chunk (bit-exact copy, no re-encoding)
    function packChunk(fileId, buffer) {
        const src = new Uint8Array(buffer);
        const out = new Uint8Array(HEADER_LEN + src.byteLength);
        for (let i = 0; i < Math.min(fileId.length, HEADER_LEN); i++) {
            out[i] = fileId.charCodeAt(i);
        }
        out.set(src, HEADER_LEN);
        return out.buffer;
    }
